...
import {getEventVenuesCount, getEventVenues, removeVenue} from '../../api'
...
export default class Venue extends React.Component {
    ...
    showDeleteConfirm() {
        confirm({
            ...
            cancelText: 'No',
            onOk: () => {
                this.removeVenue(this.state.activeRecord.id)
            },
            onCancel() {
            }
        });
        ...
        const columns = [{
            ...
            render: (text, record, index) => (
                <Row style={{width: '80px'}} type="flex" justify="space-around" align="middle">
                    ...
                    <Col span={12}>
                        <Popconfirm title="Are you sure delete the user?" onConfirm={() => this.removeVenue(record.id)} okText="Yes" cancelText="No">
        ...
        return (
            <div style={{height: '100%'}}>
                ...
                <AddVenue 
                    ...
                    eventId={params.id}/>
                <EditVenue
                    venue={this.state.activeRecord}
                    hideEditDialog={this.hideEditDialog}
                    visible={this.state.showEditModal}/>
                <FilterTable 
                    uuid={this.state.uuid}
                    columns={columns}
                    fetchCount={this.fetchVenuesCount}
                    filterData={this.fetchVenues}/>
            </div>
        )
    }
}
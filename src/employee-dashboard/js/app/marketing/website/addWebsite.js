...
import {getSpeakers, addSpeaker, tagSpeakers} from '../../api';
const { Option } = Select;
...
export default class AddSpeaker extends React.Component {
    ...
    tagSpeakers() {
        var eventSpeakers = this.state.eventSpeakers.map(el => ({id: el.id, type: el.type}))
        this.setState({ loading: true })
        tagSpeakers({
            event_id: this.props.eventId,
            speakers: eventSpeakers
        }).then(data => {
            this.setState({ loading: true, eventSpeakers: [] })
            this.props.fetchSpeakers();
            this.props.onClose();
        }).catch(err => {
            this.setState({ loading: true })
